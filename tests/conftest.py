...
- Module-level collection references in clip_service / export_service (created
  at import time before any fixture runs) are replaced with a fresh mongomock
  collection for each test.
"""
...
def mongo_patch(monkeypatch):
    ...
    from services import clip_service, export_service, email_service
    ...
    monkeypatch.setattr(export_service, "col", fake_col)
